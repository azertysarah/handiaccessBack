...
router.post('/signup', async (req: Request, res: Response) => {
  ...
  if (email !== confirm_email) {
    return res.status(400).json({ message: "L'adresse e-mail de confirmation ne correspond pas à l'adresse e-mail saisie." });
  }

  // Vérifiez si les champs password et confirm_password correspondent
  if (password !== confirm_password) {
    return res.status(400).json({ message: 'Le mot de passe de confirmation ne correspond pas au mot de passe saisi.' });
  }
  ...
  try {
    ...
    return res.status(201).json({ message: 'Compte utilisateur créé avec succès.' });
  } catch (error) {
    ...
    return res.status(500).json({ message: 'Une erreur s\'est produite lors de la création du compte utilisateur.' });
  }
});
